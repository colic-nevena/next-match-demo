...
export const usePresenceChannel = () => {
    const { set, add, remove } = usePresenceStore(state => ({ // state is current state of the created store and this way we access it
        set: state.set,
        add: state.add,
        remove: state.remove
    }))
    ...
    const handleSetMembers = useCallback((memberIds: string[]) => {
        set(memberIds) // look at what the set method is doing inside the usePresenceStore
    }, [set])
